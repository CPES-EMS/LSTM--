...
try:
    ...
    raw_data2 = GetNearestDataFromDB(position_name='a', position_id='1', device_name='b', device_id='2',
                                     ...
                                     start_time=None)
    raw_data2_re = raw_data2[::-1]
    print(raw_data2_re)
    df['light_intensity'] = raw_data2_re['light_intensity']
    df['temperature'] = raw_data2_re['temperature']
    df['huimidity'] = raw_data2_re['huimidity']
    df['light_intensity'] = df['light_intensity'].values[::-1].astype(float)
    df['temperature'] = df['temperature'].values[::-1].astype(float)
    df['huimidity'] = df['huimidity'].values[::-1].astype(float)
    raw_data = df
    print(raw_data)
...
predict_time = [(time_before + datetime.timedelta(hours=4))]
...
try:
    ...
    for i in range(4):
        data_dict = {'system_id': '3', 'system_name': 'c', 'actual_time': predict_time[0][i],
                     'forcast_value': predictions[4-i-1], 'forcast_type': '4'}
