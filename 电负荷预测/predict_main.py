...
try:
    # 从电气系统数据库读入数据
    row_name = ['load_time', 'load_value']
    raw_data = GetAllDataFromDB(position_name='a', position_id='1', device_name='b', device_id='2',
                                system_name='c',
                                system_id='3', row_name=','.join(row_name), table_name='sheet1', time='load_time',
                                start_time=None)

    arr = np.array(raw_data['load_time'])
    ...
    df['load_value'] = raw_data['load_value'][::-1].astype(float)
    raw_data = df
